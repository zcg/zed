...
Zed is a high-performance, multiplayer code editor written in Rust, created by the makers of Atom and Tree-sitter. It uses GPUI, a custom GPU-accelerated UI framework (Metal on macOS, Vulkan on Windows).
...
| `cargo test -p <crate> <test_name>` | Run single test |
| `./script/clippy` (Unix) / `./script/clippy.ps1` (Windows) | Run clippy (use this, NOT `cargo clippy`) |
| `cargo xtask clippy` | Alternative clippy via xtask |
...
The codebase is a Cargo workspace with ~200 crates in `/crates`. Key crates:

| Crate | Responsibility |
|-------|----------------|
| `gpui` | GPU-accelerated UI framework - start here for UI understanding |
| `zed` | Main entry point, brings everything together |
| `editor` | Core Editor type for code editing and input fields |
| `project` | File/navigation management, LSP client |
| `workspace` | State serialization, project grouping |
| `vim` | Vim workflow implementation over editor |
| `language` | Language understanding, syntax map, symbols |
| `lsp` | External LSP server communication |
| `collab` | Collaboration server |
| `rpc` | Collaboration message definitions |
| `theme` | Theme system and default themes |
| `ui` | Reusable UI components |
| `cli` | Command-line interface |

Other directories:
- `/extensions` - Built-in extensions (languages, themes)
- `/script` - Build and CI scripts
- `/tooling/xtask` - Task runner for clippy, licenses, workflows
- `/assets` - Keymaps, fonts, icons, themes
...
- Requires: Visual Studio with MSVC C++ build tools, Spectre-mitigated libs, Windows 10/11 SDK (10.0.20348.0+), CMake
- Enable long paths: `git config --system core.longpaths true`
- Graphics API: Vulkan
- If `RUSTFLAGS` env var is set, it overrides `.cargo/config.toml` and may break builds
